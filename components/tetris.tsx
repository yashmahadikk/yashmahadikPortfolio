...
export default function Tetris() {
  ...
  return (
    <div className="w-full max-w-md rounded-2xl border border-border bg-background p-4 sm:p-6">
      ...
      <div className="relative mx-auto aspect-[1/2] w-full max-w-[280px] rounded-xl border-2 border-foreground bg-muted/20 p-1">
        ...
        {(!hasStarted || isPaused || isOver) && <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/90 px-6 text-center"><p className="font-semibold text-foreground">{!hasStarted ? 'Ready?' : isOver ? 'Game over' : 'Paused'}</p><p className="mt-2 text-sm text-muted-foreground">{!hasStarted ? 'Start when you are ready.' : isOver ? `Final score: ${score}` : 'The board is waiting.'}</p><button onClick={!hasStarted ? startGame : isOver ? restart : () => setIsPaused(false)} className="mt-4 rounded-xl border border-foreground px-4 py-2 text-sm text-foreground hover:bg-foreground hover:text-background">{!hasStarted ? 'Start game' : isOver ? 'Play again' : 'Resume'}</button></div>}
      </div>
      <div className="mt-5 flex flex-wrap justify-center gap-2">
        <button aria-label="Move left" onClick={() => move(-1, 0)} className="rounded-lg border border-border px-4 py-2 text-foreground hover:bg-muted">←</button><button aria-label="Move down" onClick={() => move(0, 1)} className="rounded-lg border border-border px-4 py-2 text-foreground hover:bg-muted">↓</button><button aria-label="Move right" onClick={() => move(1, 0)} className="rounded-lg border border-border px-4 py-2 text-foreground hover:bg-muted">→</button><button aria-label="Rotate piece" onClick={() => rotatePiece(1)} className="border border-border p-2 text-foreground hover:bg-muted"><RotateCw size={18} /></button><button aria-label="Rotate piece counterclockwise" onClick={() => rotatePiece(-1)} className="border border-border p-2 text-foreground hover:bg-muted"><RotateCcw size={18} /></button>
      </div>
